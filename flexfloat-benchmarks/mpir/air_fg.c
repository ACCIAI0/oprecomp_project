...
int main()
{
	...
	samp = 10;	//number of test matrices
	success = 0;
...
void GEPP(double **A, double **L, double **U, double **P, int n, int mant_L, mant_U)
{
	double temp;
	int i, j, k;
	...
	{
		for(j=i+1;j<n;j++)
		{L[j][i] = my_div(U[j][i],U[i][i],mant_L); //Lower Triangular matrix formation

			for(k=0;k<n;k++)
			{
	 		 U[j][k] = my_sub(U[j][k],my_mult(U[i][k],L[j][i],mant_U),mant_U);}
	 		 }
 	}
...
void SOL(double **L, double **U, double **P, double *x, double *b, int n, int mant, double *tmp1)
{
	...
	for(i=0;i<n;i++)
	{
		for(j=0;j<n;j++)
		{
			x[i] = my_add(my_mult(P[i][j],b[j],mant_X),x[i],mant_X);
		}}
		...
		{ temp = 0;
			for(j=0;j<i;j++)
			{temp = my_add(temp,my_mult(L[i][j],tmp1[j],mant_TEMP),mant_TEMP);}
			 tmp1[i] = my_sub(x[i],temp,mant_TMP1);
		}
	...
	// x will be updated again to a solution
		...
		{ temp = 0;
			...
			x[i] = my_div(my_sub(tmp1[i],temp,mant_X),U[i][i],mant_X);
		}
	}


void MATVEC (double **A, double *x, double *p, int n, int mant_P)
{
	int i,j;
	...
	{
		for(j=0;j<n;j++)
		{ p[i] = my_add(p[i],my_mult(A[i][j],x[j],mant_P),mant_P);}
	}
}
