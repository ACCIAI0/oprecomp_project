...
__global__
void stencil_norm(float *grid, half2 *arraynorm, int nx, int ny)
{
  int index=blockIdx.x * blockDim.x +threadIdx.x; // globEl thread id
  
  int nrow=index*2/ny;
  int diff=index*2-(nrow*ny);
  int k=(nrow+1)*(ny+2)+diff+1;

  if (index<nx*ny)
  {
  	 float val1 = grid[k]*4.0f-grid[k-1]-grid[k+1] - grid[k-(ny+2)] - grid[k+(ny+2)];
  	 float val2 = grid[k+1]*4.0f-grid[k]-grid[k+2] - grid[k+1-(ny+2)] - grid[k+1+(ny+2)];
  	 half2 temp = __floats2half2_rn(val1, val2);
     arraynorm[index] = __hmul2(temp, temp);
  }
...
__global__
void reduce(half* g_idata, REAL *g_odata, int nx, int ny) {
extern __shared__ REAL sdata[];
  ...
  if ( (i+blockDim.x) < (nx*ny) ) 
     sdata[tid]=__half2float(g_idata[i])+__half2float(g_idata[i+blockDim.x]);
  else
     sdata[tid]=0.0;
...
//    MAIN LOOP 
  ...
  for (iter=0; iter<MAX_ITER; iter++) {
    ...
    stencil_norm<<<numBlocks/2,blockSize>>>(d_grid,(half2*)d_arraynorm,nx,ny); 
